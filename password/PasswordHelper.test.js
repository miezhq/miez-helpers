...
describe('PasswordHelper test suit', () => {
  describe('#hash() - password hashing', () => {
    it('should return a hash and a salt from a plain string', done => {
      passwordHelper.hash('P@ssw0rd!', (err, hash, salt) => {
        ...
        hash.length.should.equal(512);
        done();
      });
    });

    it('should return only a hash from a plain string if salt is given as an option', done => {
      passwordHelper.hash('P@ssw0rd!', { salt: 'secret salt' }, (err, hash, salt) => {
        if (err) throw err;
